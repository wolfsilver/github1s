...
export class PullRequestCommentController implements CommentHandler, CommentReactionHandler {
	...
	private getPREditors(editors: readonly vscode.TextEditor[]): vscode.TextEditor[] {
		return editors.filter((editor) => {
			...
			if (!params || params.id !== this.pullRequestModel.number) {
				return false;
			}
	...
	private getCommentThreadCacheKey(fileName: string, isBase: boolean): string {
		return `${fileName}-${isBase ? 'base' : 'head'}`;
	}
	...
	private onDidChangeReviewThreads(e: ReviewThreadChangeEvent): void {
		e.added.forEach(async (thread) => {
			...
			if (index > -1) {
				...
				newThread.comments = thread.comments.map(
					(c) => new GHPRComment(c, newThread!, this.pullRequestModel.githubRepository)
				);
				...
				if (matchingEditor) {
					...
					const range = threadRange(
						thread.startLine - 1,
						endLine,
						matchingEditor.document.lineAt(endLine).range.end.character
					);
		...
		e.changed.forEach((thread) => {
			const key = this.getCommentThreadCacheKey(thread.path, thread.diffSide === DiffSide.LEFT);
			const index = this._commentThreadCache[key]
				? this._commentThreadCache[key].findIndex((t) => t.gitHubThreadId === thread.id)
				: -1;
			if (index > -1) {
	...
	public async createOrReplyComment(
		...
		const isDraft = isSingleComment ? false : inDraft !== undefined ? inDraft : this.pullRequestModel.hasPendingReview;
	...
	public async openReview(): Promise<void> {
		await PullRequestOverviewPanel.createOrShow(
			this._folderReposManager.context.extensionUri,
