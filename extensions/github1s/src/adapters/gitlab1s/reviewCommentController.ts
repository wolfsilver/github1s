	...
	implements vscode.Disposable, CommentHandler, vscode.CommentingRangeProvider, CommentReactionHandler
{
	...
	private async createOutdatedCommentThread(path: string, thread: IReviewThread): Promise<GHPRCommentThread> {
		...
		return createVSCodeCommentThreadForReviewThread(
			...
			(await this._reposManager.getCurrentUser()).login,
			this._reposManager.activePullRequest?.githubRepository
		);
	}
	 ...
	 * @returns A GHPRCommentThread that has been created on an editor.
	 */
	private async createReviewCommentThread(
		uri: vscode.Uri,
		path: string,
		thread: IReviewThread
	): Promise<GHPRCommentThread> {
				...
				e.added.forEach(async (thread) => {
					...
					if (index > -1) {
						...
						newThread.comments = thread.comments.map(
							(c) => new GHPRComment(c, newThread, activePullRequest.githubRepository)
						);
	...
	public updateCommentExpandState(expand: boolean) {
		...
		function updateThreads(
			...
			threads: { [key: string]: GHPRCommentThread[] },
			reviewThreads: Map<string, Map<string, IReviewThread>>
		) {
	...
	async provideCommentingRanges(
		...
		if (document.uri.scheme === this._repository.rootUri.scheme) {
			...
			if (matchedFile) {
				...
				if (ranges.length === 0) {
					Logger.debug(
						'No commenting ranges: File has diffs, but they could not be mapped to current lines.',
						ReviewCommentController.ID
					);
				}
			} else {
				Logger.debug(
					'No commenting ranges: File does not match any of the files in the review.',
					ReviewCommentController.ID
				);
			}

			Logger.debug(
				`Providing ${ranges.length} commenting ranges for ${nodePath.basename(document.uri.fsPath)}.`,
				ReviewCommentController.ID
			);
			return ranges;
		} else {
		...
		try {
			if (!hasExistingComments) {
				...
				if (comment instanceof GHPRComment) {
					await this._reposManager.activePullRequest!.createCommentReply(input, comment.rawComment.graphNodeId, false);
				} else {
	...
	async createOrReplyComment(
		...
		try {
			...
			if (e.graphQLErrors?.length && e.graphQLErrors[0].type === 'NOT_FOUND') {
				vscode.window
					.showWarningMessage("The comment that you're replying to was deleted. Refresh to update.", 'Refresh')
					.then((result) => {
						if (result === 'Refresh') {
	...
	async toggleReaction(comment: GHPRComment, reaction: vscode.CommentReaction): Promise<void> {
		try {
			...
			if (
				...
				await this._reposManager.activePullRequest.addCommentReaction(comment.rawComment.graphNodeId, reaction);
			} else {
				await this._reposManager.activePullRequest.deleteCommentReaction(comment.rawComment.graphNodeId, reaction);
			}
		} catch (e) {
			throw new Error(formatError(e));
		}
	...
	async applySuggestion(comment: GHPRComment) {
		...
		const editor = vscode.window.visibleTextEditors.find(
			(editor) => comment.parent.uri.toString() === editor.document.uri.toString()
		);
		if (!editor) {
