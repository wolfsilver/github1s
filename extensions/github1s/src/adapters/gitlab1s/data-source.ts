...
export class GitLab1sDataSource extends DataSource {
	...
	async provideCodeReviews(
		...
		return data.map((item) => ({
			...
			mergeTime: (item.merged_at || item.updated_at) ? new Date(item.merged_at || item.updated_at) : null,
			closeTime: (item.closed_at || item.updated_at) ? new Date(item.closed_at || item.updated_at) : null,
