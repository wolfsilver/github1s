...
export class ReviewManager {
	...
	private async offerIgnoreBranch(currentBranchName): Promise<boolean> {
		...
		if (currentTime - lastOfferTime < 1000 * 60 * 60 * 24) {
			// 1 day
			return false;
		}
		...
		const offerResult = await vscode.window.showInformationMessage(
			vscode.l10n.t(
				`There\'s a pull request associated with the default branch '{0}'. Do you want to ignore this Pull Request?`,
				currentBranchName
			),
			ignore,
			dontShow
	...
	private async checkGitHubForPrBranch(
		branchName: string
	): Promise<(PullRequestMetadata & { model: PullRequestModel }) | undefined> {
		...
		const metadataFromGithub = await this._folderRepoManager.getMatchingPullRequestMetadataFromGitHub(
			this.repository.state.HEAD?.upstream?.remote,
			this._repository.state.HEAD?.upstream?.name
		);
		if (metadataFromGithub) {
			await PullRequestGitHelper.associateBranchWithPullRequest(this._repository, metadataFromGithub.model, branchName);
	...
	): Promise<(PullRequestModel & IResolvedPullRequestModel) | undefined> {
	...
	private async validateState(silent: boolean, updateLayout: boolean) {
		...
		const useReviewConfiguration = vscode.workspace
								...
								for (const localChange of this._reviewModel.localFileChanges) {
									...
									if (localChange.fileName === fileName.path) {
										...
										vscode.window.tabGroups
											.close(tab)
											.then((_) => localChange.openDiff(this._folderRepoManager, { preview: tab.isPreview }));
										break;
									}
								}
							}
	...
	private _doFocusShow(pr: PullRequestModel, updateLayout: boolean) {
		...
		if (updateLayout) {
			const focusedMode = vscode.workspace
				.getConfiguration(SETTINGS_NAMESPACE)
				.get<'firstDiff' | 'overview' | false>(FOCUSED_MODE);
	...
	private async getLocalChangeNodes(
		...
		for (let i = 0; i < contentChanges.length; i++) {
			...
			const changeModel = new GitFileChangeModel(
				...
				originalFileUri,
				headSha,
				contentChanges.length < 20
			);
			const changedItem = new GitFileChangeNode(this.changesInPrDataProvider, this._folderRepoManager, pr, changeModel);
	...
	private async initializePullRequestData(pr: PullRequestModel & IResolvedPullRequestModel): Promise<void> {
		try {
			...
			await Promise.all([
				pr.initializeReviewComments(),
				pr.initializeReviewThreadCache(),
	...
	public async switch(pr: PullRequestModel): Promise<void> {
		...
		try {
			...
			if (e.message === 'User aborted') {
				...
				e.gitErrorCode &&
				(e.gitErrorCode === GitErrorCodes.LocalChangesOverwritten || e.gitErrorCode === GitErrorCodes.DirtyWorkTree)
			) {
				...
				vscode.window.showInformationMessage(
					'The remote branch for this pull request has been deleted. The file contents may not match the remote.'
				);
			} else {
	...
	public async publishBranch(branch: Branch): Promise<Branch | undefined> {
		...
		if (
			...
			selectedRemote = (await this._folderRepoManager.getGitHubRemotes()).find(
				(element) => element.remoteName === fork
			);
		}
		...
		return new Promise<Branch | undefined>(async (resolve) => {
			...
			const validate = async function (value: string) {
				try {
					...
					if (remoteBranch) {
						inputBox.validationMessage = vscode.l10n.t(
							`Branch '{0}' already exists in {1}`,
							value,
			...
			inputBox.onDidAccept(async () => {
				...
				try {
					...
					if (err.gitErrorCode === GitErrorCodes.RemoteConnectionError) {
						vscode.window.showWarningMessage(
							vscode.l10n.t(
								`Could not read from remote repository '{0}'. Please make sure you have the correct access rights and the repository exists.`,
								remote.remoteName
							),
							{
								modal: true,
	...
	public async createPullRequest(compareBranch?: string): Promise<void> {
		if (!this._createPullRequestHelper) {
			...
			this._createPullRequestHelper.onDidCreate(async (createdPR) => {
				const postCreate = vscode.workspace
					.getConfiguration(SETTINGS_NAMESPACE)
					.get<'none' | 'openOverview' | 'checkoutDefaultBranch'>(POST_CREATE, 'openOverview');
	...
	public async openDescription(): Promise<void> {
		...
		const descriptionNode = this.changesInPrDataProvider.getDescriptionNode(this._folderRepoManager);
		await openDescription(this._context, this._telemetry, pullRequest, descriptionNode, this._folderRepoManager);
	}
	...
	async provideTextDocumentContent(uri: vscode.Uri): Promise<string | undefined> {
		...
		if (changedItems.length) {
			...
			const content = await getGitHubFileContent(
				this._folderRepoManager.activePullRequest.githubRepository,
				path,
				commit
			);
	...
	static getReviewManagerForRepository(
		...
		return reviewManagers.find((reviewManager) =>
			reviewManager._folderRepoManager.gitHubRepositories.some((repo) => {
				...
				return (
					repo.equals(githubRepository) && (!repository || reviewManager._folderRepoManager.repository === repository)
				);
			})
		);
	}
